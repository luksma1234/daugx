...
from daugx.utils.misc import string_to_list, list_intersection, is_header, is_in_dict, img_dims, new_id
import numpy as np
import yaml
import os
from daugx.core.data.new_data.data import DataItem, AnnotationItem, ImageItem
import json
import xmltodict
import csv
import cv2
...
class DataLoader(ABC):
    ...
    def preload(self) -> List[DataItem]:
        """
        This method preloads data. May not be necessary.
        """
        pass
    ...
    @abstractmethod
    def _transform(self, raw_data: dict) -> DataItem:
        """
        Transforms a loaded data dict to a DataItem object.
        """
        pass
...
class ImageLoader(DataLoader):
    ...
    def __init__(self, rng: np.random.Generator, directory: str, types: List[str], recursive: bool = False):
        """
        Args:
            directory (str): Directory of image folder.
            types (List[str]): List of accepted file types/extensions which should be loaded from directory.
            rng (np.random.Generator): RNG for data IDs
    ...
    def load(self, preloaded_item: ImageItem) -> ImageItem:
...
class AQL:
    ...
    def _separate(self):
        """
        Separates query string into keywords and arguments.
        """
    ...
    def _index_loading_queries(self):
        ...
        for loading_query in self.__arguments:
...
# TODO: Define __next__ as abstractmethod in loader?
# TODO: Any Loader returns a List[DataItem] with its load method.
class InitialLoader:
    """
